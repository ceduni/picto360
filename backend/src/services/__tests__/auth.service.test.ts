...
describe("AuthService", () => {
  ...
  describe("generateAuthUrl", () => {
    ...
    it("responds with the auth URL generated by the active provider", async () => {
      const request = { body: {}, session: { save: jest.fn<() => Promise<void>>().mockResolvedValue(undefined) } } as unknown as jest.Mocked<FastifyRequest>;
      const reply   = makeReply();

      await authService.generateAuthUrl(request, reply);

      expect(mockAuthProvider.generateAuthUrl).toHaveBeenCalledWith(expect.any(String));
      expect(reply.status).toHaveBeenCalledWith(200);
      expect(reply.send).toHaveBeenCalledWith({ authUrl: "https://accounts.google.com/oauth/authorize?..." });
    });

    it("stores the generated state on the session for later CSRF validation", async () => {
      const session = { save: jest.fn<() => Promise<void>>().mockResolvedValue(undefined) } as any;
      const request = { body: { returnTo: "/gallery", viewerId: "viewer-1" }, session } as unknown as jest.Mocked<FastifyRequest>;
      const reply   = makeReply();

      await authService.generateAuthUrl(request, reply);

      const [state] = mockAuthProvider.generateAuthUrl.mock.calls[0];
      expect(session.oauth_state).toBe(state);
      expect(session.oauth_metadata).toEqual(
        expect.objectContaining({ returnTo: "/gallery", viewerId: "viewer-1" })
      );
    });

    it("returns 500 when the provider fails to generate a URL", async () => {
      mockAuthProvider.generateAuthUrl = jest.fn<() => string>().mockImplementation(() => {
      ...
      expect(reply.send).toHaveBeenCalledWith({ error: "Failed to generate auth URL" });
    });
  });

  describe("exchangeCodeToToken", () => {
    it("should exchange code for tokens", async () => {
      const result = await authService.exchangeCodeToToken(mockRequest, "auth-code-123");
    ...
    it("should retrieve user info after getting tokens", async () => {
      await authService.exchangeCodeToToken(mockRequest, "auth-code-123");
    ...
    it("should handle token exchange failure", async () => {
      ...
      await expect(authService.exchangeCodeToToken(mockRequest, "invalid-code")).rejects.toThrow(
    ...
    it("should handle user info retrieval failure", async () => {
      ...
      await expect(authService.exchangeCodeToToken(mockRequest, "auth-code-123")).rejects.toThrow(
